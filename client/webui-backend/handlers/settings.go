...
func GetSettings(w http.ResponseWriter, r *http.Request) {
	...
	// 本机网络：webui.json 未记录时（老部署）实时探测系统现状用于显示
	lanIface, lanCIDR, lanGW := cfg.LanIface, cfg.LanIPCIDR, cfg.LanGateway
	if lanCIDR == "" {
		di, dc, dg := services.DetectCurrentNetwork()
		if lanIface == "" {
			lanIface = di
		}
		lanCIDR = dc
...
func UpdateSettings(w http.ResponseWriter, r *http.Request) {
	var req struct {
		...
		LanIPCIDR    *string `json:"lan_ip_cidr"`
	...
	// 本机静态 IP / 网关：任一字段提供且与现值不同则校验并改写网卡配置
	if req.LanIPCIDR != nil || req.LanGateway != nil || req.LanIface != nil {
		newCIDR := cfg.LanIPCIDR
		if req.LanIPCIDR != nil {
			newCIDR = strings.TrimSpace(*req.LanIPCIDR)
		}
		newGW := cfg.LanGateway
		if req.LanGateway != nil {
			newGW = strings.TrimSpace(*req.LanGateway)
		}
		newIface := cfg.LanIface
		if req.LanIface != nil {
			newIface = strings.TrimSpace(*req.LanIface)
		}
		if newCIDR != cfg.LanIPCIDR || newGW != cfg.LanGateway || newIface != cfg.LanIface {
			if newIface == "" {
				writeJSON(w, http.StatusBadRequest, errorMsg("网卡接口不能为空"))
				return
			}
			if err := services.ValidateLanCIDR(newCIDR); err != nil {
				writeJSON(w, http.StatusBadRequest, errorMsg(err.Error()))
				return
			}
			if err := services.ValidateLanGateway(newGW, newCIDR); err != nil {
				writeJSON(w, http.StatusBadRequest, errorMsg(err.Error()))
				return
			}
			cfg.LanIface = newIface
			cfg.LanIPCIDR = newCIDR
			cfg.LanGateway = newGW
			needRestartNet = true
		}
	}
	...
	if needRestartNet {
		if err := services.WriteInterfacesFile(cfg.LanIface, cfg.LanIPCIDR, cfg.LanGateway); err != nil {
			writeJSON(w, http.StatusInternalServerError, errorMsg("写入网卡配置失败: "+err.Error()))
			return
		}
	}
	...
	if needUpdateTimer {
		// 更新 systemd timer 的 OnCalendar（固定北京时间）
		_ = services.WriteUpdateTimer(cfg.UpdateHour, cfg.UpdateMinute)
	}
	...
	if needRestartNet {
		resp["network_will_restart"] = true
		// 异步重启 networking，IP 变更会断开当前连接
		services.RestartNetworking()
	}
...
func EmergencyStop(w http.ResponseWriter, r *http.Request) {
	runEmergencyScript(w, "/opt/tproxy-gw/scripts/emergency-stop.sh")
}

// EmergencyResume POST /api/emergency-resume
func EmergencyResume(w http.ResponseWriter, r *http.Request) {
	runEmergencyScript(w, "/opt/tproxy-gw/scripts/emergency-resume.sh")
}

func runEmergencyScript(w http.ResponseWriter, path string) {
