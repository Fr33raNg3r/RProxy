...
func CreateNode(w http.ResponseWriter, r *http.Request) {
	...
	n.Order = len(nodes)
	applyNodeDefaults(&n)
	// Enabled 由 CurrentNodeID 同步管理，新建节点默认不启用
	n.Enabled = false

	// 如果当前没有活动节点（首次添加），自动选定为当前并启用
	cfg, _ := config.LoadWebUIConfig()
	firstActive := cfg != nil && cfg.CurrentNodeID == ""
	if firstActive {
		n.Enabled = true
	}
...
func UpdateNode(w http.ResponseWriter, r *http.Request) {
	...
	for i := range nodes {
		if nodes[i].ID == id {
			...
			// Enabled 由 CurrentNodeID 同步管理，编辑不允许修改
			n.Enabled = nodes[i].Enabled
			applyNodeDefaults(&n)
			nodes[i] = n
			found = true
			break
...
func DeleteNode(w http.ResponseWriter, r *http.Request) {
	...
	// 如果删除的是当前节点，按 order 选一个新的作为 current
	cfg, _ := config.LoadWebUIConfig()
	if cfg != nil && cfg.CurrentNodeID == id {
		cfg.CurrentNodeID = ""
		// 按 order 选第一个剩余节点
		...
		for i := range newNodes {
			if nextIdx == -1 || newNodes[i].Order < newNodes[nextIdx].Order {
				nextIdx = i
			}
		}
		if nextIdx >= 0 {
			cfg.CurrentNodeID = newNodes[nextIdx].ID
		}
		// 同步 Enabled
		for i := range newNodes {
			newNodes[i].Enabled = newNodes[i].ID == cfg.CurrentNodeID
		}
		if err := config.SaveNodes(newNodes); err != nil {
			writeJSON(w, http.StatusInternalServerError, errorMsg("保存节点失败: "+err.Error()))
			return
		}
		if err := config.SaveWebUIConfig(cfg); err != nil {
...
func SwitchNode(w http.ResponseWriter, r *http.Request) {
	...
	// 同步 Enabled：只有目标节点是 true
	for i := range nodes {
		nodes[i].Enabled = nodes[i].ID == id
	}
	if err := config.SaveNodes(nodes); err != nil {
		writeJSON(w, http.StatusInternalServerError, errorMsg(err.Error()))
		return
