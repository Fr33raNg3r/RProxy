...
# 客户端 sysctl 配置（包含 conntrack 调大，旁路由场景需要）
RPROXY_SYSCTL_FILE="/etc/sysctl.d/99-rproxy.conf"
...
# ---- UDP 缓冲区（旁路由 TPROXY 大量 UDP 流量）----
net.core.netdev_budget = 600
net.ipv4.udp_rmem_min = 16384
net.ipv4.udp_wmem_min = 16384

# ---- Conntrack（透明代理产生大量连接，必须调大）----
...
EOF
    ...
    echo "    conntrack_max：$(sysctl -n net.netfilter.nf_conntrack_max 2>/dev/null)"
    echo ""
...
system_optimize_menu() {
    while true; do
        clear
        echo -e "${CYAN}========================================================================${NC}"
        echo -e "${CYAN}                       RProxy 客户端 - 系统优化${NC}"
        echo -e "${CYAN}========================================================================${NC}"
        echo ""
