...
# 交互式收集本机静态 IP / 网关（要求内网 CIDR），结果写入全局变量
# LAN_IFACE / LAN_IP_CIDR / LAN_GATEWAY
prompt_network_config() {
    local iface detected_cidr detected_gw in
    iface=$(get_default_iface)
    [[ -n "$iface" ]] || iface="eth0"
    detected_cidr=$(detect_lan_cidr "$iface")
    detected_gw=$(detect_lan_gateway)

    echo ""
    echo -e "${CYAN}── 本机网络配置（静态 IP）─────────────────────────────${NC}"
    echo "  旁路由需要一个固定的内网 IP，下列配置将写入网卡（/etc/network/interfaces）。"
    echo -e "  IP 必须为${YELLOW}内网地址${NC}且使用${YELLOW} CIDR ${NC}格式，例如：${CYAN}192.168.1.2/24${NC}"
    echo ""

    in=$(ask "  网卡接口 [默认 ${iface}]: " "$iface")
    iface="${in:-$iface}"

    while true; do
        in=$(ask "  本机 IP 地址(CIDR)${detected_cidr:+ [默认 ${detected_cidr}]}: " "$detected_cidr")
        if validate_lan_cidr "$in"; then
            LAN_IP_CIDR="$in"
            break
        fi
    done
    ...
    while true; do
        in=$(ask "  默认网关${detected_gw:+ [默认 ${detected_gw}]}: " "$detected_gw")
        if validate_lan_gateway "$in" "$LAN_IP_CIDR"; then
            LAN_GATEWAY="$in"
            break
        fi
    done
    ...
    LAN_IFACE="$iface"
    echo ""
    log_info "网络配置：接口=${LAN_IFACE}  IP=${LAN_IP_CIDR}  网关=${LAN_GATEWAY}"
    echo ""
}

# 把收集到的静态网络配置写入网卡（不立即重启网络，避免中断安装中的 SSH）
configure_network() {
    [[ -n "${LAN_IP_CIDR:-}" && -n "${LAN_GATEWAY:-}" && -n "${LAN_IFACE:-}" ]] || return 0
    log_step "写入本机静态网络配置到 /etc/network/interfaces"
    write_interfaces_file "$LAN_IFACE" "$LAN_IP_CIDR" "$LAN_GATEWAY"
    log_done "网卡配置已写入（重启网络后生效）"
}

# 应用静态网络配置——重启 networking。IP 变更会断开当前 SSH/WebUI 连接。
apply_network_config() {
    [[ -n "${LAN_IP_CIDR:-}" ]] || return 0
    log_step "应用本机网络配置（重启 networking，连接可能中断）"
...
# ensure_xray_log_dir 保证 /var/log/xray 存在、权限可用，并清理旧 log 文件。
# 必须在 Xray 启动/重启之前调用——Xray rendered config 硬写了 access.log/error.log，
# 打开失败 = 服务起不来。install 和 upgrade 都要调，避免遗留所有权异常的旧文件。
ensure_xray_log_dir() {
    mkdir -p /var/log/xray
    chown nobody:nogroup /var/log/xray 2>/dev/null || chown nobody /var/log/xray
    chmod 755 /var/log/xray
    # 清理旧 log 文件，让 Xray 以当前 user 重新创建。
    # 历史上 Xray 在 nobody / root 两种 user 间切换过，旧文件所有权可能与当前不匹配。
...
# 防御性放行 /var/log/xray 写入：
# 上游 Xray-install 生成的 unit 可能带 ProtectSystem=strict 之类沙箱，
# 那会让 /var/log/xray 即便对 root 也只读，导致 Xray 打开 access.log/error.log 失败。
# ReadWritePaths 是累加的——加一行不会破坏原 unit 的安全配置。
...
deploy_scripts_and_configs() {
    ...
    cp "${BUILD_DIR}/scripts/load-cn-ips.sh" "${SCRIPTS_DIR}/"
    cp "${BUILD_DIR}/scripts/load-static-ips.sh" "${SCRIPTS_DIR}/"
...
EOF
    ...
    # ----- mosdns 配置 -----
    # 模板嵌入在 webui 二进制里；render-mosdns 会在 upstreams.json 不存在时写默认值
    if [[ ! -f "${CONFIG_DIR}/mosdns/config.yaml" ]]; then
        "${BIN_DIR}/webui" render-mosdns
    fi
    ...
    # 黑白名单里拆出来的静态 IP（域名走上面的 .txt + mosdns，IP 走这里 + nftables set）
    [[ -f "${CONFIG_DIR}/dns/whitelist_ips.txt" ]] || touch "${CONFIG_DIR}/dns/whitelist_ips.txt"
    [[ -f "${CONFIG_DIR}/dns/blacklist_ips.txt" ]] || touch "${CONFIG_DIR}/dns/blacklist_ips.txt"
...
enable_services() {
    ...
    # 预加载 geoip-cn 到 cn_ips：避免裸 IP 访问国内站点被错送代理
    systemctl enable --now tproxy-gw-load-cn-ips.service
    # 加载黑白名单里的静态 IP 到 whitelist_ips/blacklist_ips（开机重灌）
    systemctl enable --now tproxy-gw-load-static-ips.service
...
action_install_fresh() {
    ...
    # 先收集本机静态 IP / 网关（在长耗时下载前完成交互，避免用户等待）
    prompt_network_config
    ...
    generate_default_configs
    generate_wg_config_file
    configure_network             # 写入静态 IP/网关到网卡（先写文件，结尾才应用）
    ...
    # 最后一步：应用静态网络配置。若 IP 有变更，当前 SSH 连接会断开（安装已完成）。
    apply_network_config
}
...
action_upgrade() {
    ...
    # 配置迁移：旧版 webui.json 没有 password_changed 字段
    # 已存在的部署视为用户已自行设置过密码，避免升级后被强制改密打扰
    if [[ -f "${CONFIG_DIR}/webui.json" ]]; then
        if ! jq -e 'has("password_changed")' "${CONFIG_DIR}/webui.json" >/dev/null 2>&1; then
            local tmp="${CONFIG_DIR}/webui.json.tmp"
            jq '. + {password_changed: true}' "${CONFIG_DIR}/webui.json" > "$tmp" \
                && mv "$tmp" "${CONFIG_DIR}/webui.json" \
                && chmod 600 "${CONFIG_DIR}/webui.json"
            log_info "webui.json 已补 password_changed=true（旧部署兼容）"
        fi
        # 配置迁移：旧版没有 lan_* 字段。从当前系统探测回填，
        # 供 WebUI【设置】页显示/编辑（不改写 /etc/network/interfaces，避免扰动现有网络）
        if ! jq -e 'has("lan_ip_cidr")' "${CONFIG_DIR}/webui.json" >/dev/null 2>&1; then
            local cur_iface cur_cidr cur_gw tmp2
            cur_iface=$(get_default_iface)
            cur_cidr=$(detect_lan_cidr "$cur_iface")
            cur_gw=$(detect_lan_gateway)
            tmp2="${CONFIG_DIR}/webui.json.tmp"
            jq --arg i "$cur_iface" --arg c "$cur_cidr" --arg g "$cur_gw" \
                '. + {lan_iface: $i, lan_ip_cidr: $c, lan_gateway: $g}' \
                "${CONFIG_DIR}/webui.json" > "$tmp2" \
                && mv "$tmp2" "${CONFIG_DIR}/webui.json" \
                && chmod 600 "${CONFIG_DIR}/webui.json"
            log_info "webui.json 已回填 lan_* 网络字段（旧部署兼容）"
        fi
        # 配置迁移：旧版没有 update_hour/update_minute 字段。回填默认 04:00，
        # 避免升级后 render-update-timer 读到 0 把更新时间改成 00:00。
        if ! jq -e 'has("update_hour")' "${CONFIG_DIR}/webui.json" >/dev/null 2>&1; then
            local tmp3="${CONFIG_DIR}/webui.json.tmp"
            jq '. + {update_hour: 4, update_minute: 0}' "${CONFIG_DIR}/webui.json" > "$tmp3" \
                && mv "$tmp3" "${CONFIG_DIR}/webui.json" \
                && chmod 600 "${CONFIG_DIR}/webui.json"
            log_info "webui.json 已回填 update_hour/update_minute（旧部署兼容）"
        fi
    fi
    ...
    # 只 enable，不 start —— 启动放到 nft -f 之后，避免 cn_ips 被 flush ruleset 抹掉
    systemctl enable tproxy-gw-load-cn-ips.service 2>/dev/null || true
    systemctl enable tproxy-gw-load-static-ips.service 2>/dev/null || true
    ...
    "${BIN_DIR}/webui" render-wg || true
    # 重新生成 mosdns 配置（兼容旧版本升级——upstreams.json 不存在时自动写默认值）
    "${BIN_DIR}/webui" render-mosdns || true
    # 上面 cp *.timer 会把用户在 WebUI 改过的更新时间覆盖回模板默认 04:00，
    # 这里按 webui.json 重建 update timer（恢复自定义时间 + 写入 Asia/Shanghai 时区）
    "${BIN_DIR}/webui" render-update-timer || true

    nft -f /etc/nftables.conf
    # nft -f 含 flush ruleset，cn_ips 被清空，必须在重载后重灌
    # 用 restart 而不是 enable --now —— 后者对已 active(exited) 的 oneshot 不会再触发 ExecStart
    systemctl restart tproxy-gw-load-cn-ips.service 2>/dev/null || true
    systemctl restart tproxy-gw-load-static-ips.service 2>/dev/null || true
...
show_completion_info() {
    local ip
    # 优先用本次配置的静态 IP（去掉 CIDR 前缀），否则回退到当前实际 IP
    ...
    echo ""
    if [[ -n "${LAN_IP_CIDR:-}" ]]; then
        echo -e "  ${YELLOW}本机 IP 已配置为 ${LAN_IP_CIDR}（网关 ${LAN_GATEWAY}）。${NC}"
        echo -e "  ${YELLOW}马上会重启网络使其生效，若 IP 有变更当前连接将断开，${NC}"
        echo -e "  ${YELLOW}请用新 IP ${CYAN}${ip}${NC}${YELLOW} 重新连接。${NC}"
        echo ""
    fi
}
